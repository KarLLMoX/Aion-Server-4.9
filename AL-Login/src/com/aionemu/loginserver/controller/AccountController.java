...
public class AccountController {
    ...
    public static synchronized void checkAuth(SessionKey key, GsConnection gsConnection) {
        ...
        if (con != null && con.getSessionKey().checkSessionKey(key)) {
             ...
             */
            gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, true, acc.getName(), acc.getAccessLevel(), acc.getMembership(), toll, acc.getReturn()));
        } else {
            gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, false, null, (byte) 0, (byte) 0, 0, (byte)0));
        }
    }
